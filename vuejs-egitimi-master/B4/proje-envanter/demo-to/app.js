Vue.component('EnvanterForm', {
    ...
    methods : {
        saveItem(){
            // this.item.id = ++this.$parent.$data.lastId;
            // this.item.arsiv = false;
            // reactive olarak tanımlamak için
...
Vue.component('EnvanterList', {
    ...
    methods: {
        ...
        deleteItem(id){
            // this.$parent().$data.allItems = this.$parent().$data.allItems.filter(item => item.id !== id);
...
const App = new Vue({
    ...
    computed: {
        activeItems() {
            return this.filteredItems.filter(value => value.arsiv === false);
        },
        archievedItems() {
            return this.filteredItems.filter(value => value.arsiv === true);
        }
    },
    methods: {
        itemDeleted(id){
